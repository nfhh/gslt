...
class ucp_register
{
			...
			*/
			...
			if (!count($error))
			{
				...
				*/
				...
				// gen default avatar start ***
                global $config, $db;
                $my_prefix = $config['avatar_salt'] . '_' . $user_id;
                $bg_color = [
                ...
                $bg_index = $user_id % count($bg_color);
                $w = $h = 80;
                $im = imagecreatetruecolor($w, $h);
                $text_color = imagecolorallocate($im, 255, 255, 255);
                list($r, $g, $b) = $this->wpjam_hex2rgb($bg_color[$bg_index]);
                $backgroundColor = imagecolorallocate($im, $r, $g, $b);
                imagefill($im, 0, 0, $backgroundColor);
                // text, font and size to draw
                $text = strtoupper(mb_substr($user_row['username'], 0, 1));
                $font = './assets/fonts/Alibaba-PuHuiTi-Regular.ttf';
                $size = 30;
                ...
                imagepng($im, './images/avatars/upload/' . $my_prefix . '.png');
                imagedestroy($im);
                $sql = 'UPDATE ' . USERS_TABLE . ' SET ' . $db->sql_build_array('UPDATE', [
                        'user_avatar_type' => 'avatar.driver.upload',
                        'user_avatar' => $user_id . '_' . time() . '.png',
                        'user_avatar_width' => $w,
                        'user_avatar_height' => $h
                    ]) . ' WHERE user_id = ' . (int)$user_id;
                $db->sql_query($sql);
                // gen default avatar end ***
	...
	*/
	...
	protected function wpjam_hex2rgb($hex)
    {
        $hex = str_replace('#', '', $hex);
