...
function generate_smilies($mode, $forum_id)
{
    ...
    if ($mode == 'window') {
        ...
        $pagination->generate_template_pagination($base_url, 'pagination', 'start', $smiley_count,
            $config['smilies_per_page'], $start);
    }
     ...
     * @since 3.1.0-a1
     */
    $vars = ['mode', 'forum_id', 'display_link'];
...
function posting_gen_topic_types($forum_id, $cur_topic_type = POST_NORMAL)
{
    ...
    $topic_types = [
        ...
        'announce_global' => ['const' => POST_GLOBAL, 'lang' => 'POST_GLOBAL']
    ];
...
function posting_gen_attachment_entry($attachment_data, &$filename_data, $show_attach_box = true)
{
    ...
    if (count($attachment_data)) {
        ...
        foreach ($attachment_data as $count => $attach_row) {
            ...
            $download_link = append_sid("{$phpbb_root_path}download/file.$phpEx",
                'mode=view&amp;id='.(int) $attach_row['attach_id'], true,
                ($attach_row['is_orphan']) ? $user->session_id : false);

            $attachrow_template_vars[(int) $attach_row['attach_id']] = [
         ...
         * @since 3.2.2-RC1
         */
        $vars = ['attachment_data', 'attachrow_template_vars'];
...
function load_drafts($topic_id = 0, $forum_id = 0, $id = 0, $pm_action = '', $msg_id = 0)
{
     ...
     * @since 3.1.0-RC3
     */
    $vars = ['draft_rows', 'topic_ids', 'topic_rows'];
    ...
    foreach ($draft_rows as $draft) {
        ...
        if (isset($topic_rows[$draft['topic_id']])
            && (
                ($topic_rows[$draft['topic_id']]['forum_id'] && $auth->acl_get('f_read',
                        $topic_rows[$draft['topic_id']]['forum_id']))
                ||
            ...
            $link_topic = true;
            $view_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx",
                'f='.$topic_forum_id.'&amp;t='.$draft['topic_id']);
            $title = $topic_rows[$draft['topic_id']]['topic_title'];

            $insert_url = append_sid("{$phpbb_root_path}posting.$phpEx",
                'f='.$topic_forum_id.'&amp;t='.$draft['topic_id'].'&amp;mode=reply&amp;d='.$draft['draft_id']);
        } else {
            if ($draft['forum_id'] && $auth->acl_get('f_read', $draft['forum_id'])) {
                $link_forum = true;
                $view_url = append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f='.$draft['forum_id']);
                $title = $draft['forum_name'];

                $insert_url = append_sid("{$phpbb_root_path}posting.$phpEx",
                    'f='.$draft['forum_id'].'&amp;mode=post&amp;d='.$draft['draft_id']);
            } else {
                // Either display as PM draft if forum_id and topic_id are empty or if access to the forums has been denied afterwards...
                $link_pm = true;
                $insert_url = append_sid("{$phpbb_root_path}ucp.$phpEx",
                    "i=$id&amp;mode=compose&amp;d={$draft['draft_id']}".(($pm_action) ? "&amp;action=$pm_action" : '').(($msg_id) ? "&amp;p=$msg_id" : ''));
            }
        }
...
function topic_review($topic_id, $forum_id, $mode = 'topic_review', $cur_post_id = 0, $show_quote_button = true)
{
    ...
    for ($i = 0, $end = count($post_list); $i < $end; ++$i) {
        ...
        $message = generate_text_for_display($row['post_text'], $row['bbcode_uid'], $row['bbcode_bitfield'],
            $parse_flags, true);
        ...
        $u_show_post = append_sid($phpbb_root_path.'viewtopic.'.$phpEx,
            "f=$forum_id&amp;t=$topic_id&amp;p={$row['post_id']}&amp;view=show#p{$row['post_id']}");

        $l_deleted_message = '';
        if ($row['post_visibility'] == ITEM_DELETED) {
            $display_postername = get_username_string('full', $poster_id, $row['username'], $row['user_colour'],
                $row['post_username']);
            ...
            if (!$row['post_delete_user'] || $row['post_delete_user'] == $poster_id) {
                ...
                $display_username = get_username_string('full', $row['post_delete_user'], $row['post_delete_username'],
                    $row['post_delete_user_colour']);
            }

            if ($row['post_delete_reason']) {
                $l_deleted_message = $user->lang('POST_DELETED_BY_REASON', $display_postername, $display_username,
                    $user->format_date($row['post_delete_time'], false, true), $row['post_delete_reason']);
            } else {
                $l_deleted_message = $user->lang('POST_DELETED_BY', $display_postername, $display_username,
                    $user->format_date($row['post_delete_time'], false, true));
            }
        }
        ...
        $post_row = [
            ...
            'L_IGNORE_POST' => ($row['foe']) ? sprintf($user->lang['POST_BY_FOE'],
                get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']),
                "<a href=\"{$u_show_post}\" onclick=\"phpbb.toggleDisplay('{$post_anchor}', 1); return false;\">",
                '</a>') : '',
            ...
            'USER_ID' => $row['user_id'],
            'U_MINI_POST' => append_sid("{$phpbb_root_path}viewtopic.$phpEx",
                    'p='.$row['post_id']).'#p'.$row['post_id'],
            'U_MCP_DETAILS' => ($auth->acl_get('m_info', $forum_id)) ? append_sid("{$phpbb_root_path}mcp.$phpEx",
                'i=main&amp;mode=post_details&amp;f='.$forum_id.'&amp;p='.$row['post_id'], true,
                $user->session_id) : '',
            'POSTER_QUOTE' => ($show_quote_button && $auth->acl_get('f_reply',
                    $forum_id)) ? addslashes(get_username_string('username', $poster_id, $row['username'],
                $row['user_colour'], $row['post_username'])) : '',
        ];
...
function delete_post($forum_id, $topic_id, $post_id, &$data, $is_soft = false, $softdelete_reason = '')
{
    ...
    if ($is_soft && ($post_mode != 'delete_topic')) {
        $phpbb_content_visibility->set_post_visibility(ITEM_DELETED, $post_id, $topic_id, $forum_id,
            $user->data['user_id'], time(), $softdelete_reason, ($data['topic_first_post_id'] == $post_id),
            ($data['topic_last_post_id'] == $post_id));
    } else {
        if (!$is_soft) {
            if (!delete_posts('post_id', [$post_id], false, false, false)) {
    ...
    switch ($post_mode) {
        case 'delete_topic':
            ...
            if ($is_soft) {
                $phpbb_content_visibility->set_topic_visibility(ITEM_DELETED, $topic_id, $forum_id,
                    $user->data['user_id'], time(), $softdelete_reason);
            } else {
                delete_topics('topic_id', [$topic_id], false);
        ...
        case 'delete_first_post':
            ...
            $sql_data[TOPICS_TABLE] = $db->sql_build_array('UPDATE', [
        ...
        case 'delete_last_post':
            if (!$is_soft) {
                ...
                $update_sql = update_post_information('forum', $forum_id, true);
                if (count($update_sql)) {
                    $sql_data[FORUMS_TABLE] = (($sql_data[FORUMS_TABLE]) ? $sql_data[FORUMS_TABLE].', ' : '').implode(', ',
                            $update_sql[$forum_id]);
                }
...
function submit_post(
    ...
    if (!$auth->acl_get('f_noapprove', $data_ary['forum_id'])) {
        ...
        if (isset($data_ary['post_visibility']) && $data_ary['post_visibility'] !== false) {
    ...
    if (isset($data_ary['force_approved_state'])) {
        $post_visibility = (in_array((int) $data_ary['force_approved_state'], [
            ITEM_APPROVED, ITEM_UNAPPROVED, ITEM_DELETED, ITEM_REAPPROVE
        ])) ? (int) $data_ary['force_approved_state'] : $post_visibility;
    }
    if (isset($data_ary['force_visibility'])) {
        $post_visibility = (in_array((int) $data_ary['force_visibility'], [
            ITEM_APPROVED, ITEM_UNAPPROVED, ITEM_DELETED, ITEM_REAPPROVE
        ])) ? (int) $data_ary['force_visibility'] : $post_visibility;
    }
...
str;
    ...
    switch ($post_mode) {
        ...
        case 'edit_topic':
            ...
            if ($data_ary['post_edit_reason'] || (!$auth->acl_get('m_edit',
                        $data_ary['forum_id']) && ($post_mode == 'edit' || $post_mode == 'edit_first_post'))) {
                ...
                if (!$data_ary['post_edit_reason'] && $mode == 'edit' && $auth->acl_get('m_edit',
                        $data_ary['forum_id'])) {
                    $sql_data[POSTS_TABLE]['sql'] = [
                        'post_edit_reason' => '',
                    ];
            ...
            if ($user->data['user_id'] != $poster_id) {
                ...
                $phpbb_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_POST_EDITED', false, [
                    'forum_id' => $data_ary['forum_id'],
    ...
    switch ($post_mode) {
        case 'post':
            ...
            $sql_data[USERS_TABLE]['stat'][] = "user_lastpost_time = $current_time".(($auth->acl_get('f_postcount',
                        $data_ary['forum_id']) && $post_visibility == ITEM_APPROVED) ? ', user_posts = user_posts + 1' : '');
    ...
    // Submit new post
    if ($post_mode == 'post' || $post_mode == 'reply' || $post_mode == 'at') {
        if ($post_mode == 'reply' || $post_mode == 'at') {
            $sql_data[POSTS_TABLE]['sql'] = array_merge($sql_data[POSTS_TABLE]['sql'], [
                'topic_id' => $data_ary['topic_id'],
            ]);
        }
        ...
        for ($i = 0, $size = count($poll_ary['poll_options']); $i < $size; $i++) {
            if (strlen(trim($poll_ary['poll_options'][$i]))) {
                if (empty($cur_poll_options[$i])) {
                    ...
                    if ($poll_ary['poll_options'][$i] != $cur_poll_options[$i]) {
    ...
    // Submit Attachments
    if (!empty($data_ary['attachment_data']) && $data_ary['post_id'] && in_array($mode,
            ['post', 'reply', 'at', 'quote', 'edit'])) {
    ...
    if (($post_mode != 'post' && $post_mode != 'reply' && $post_mode != 'at') && $data_ary['post_visibility'] != $post_visibility) {
        ...
        /* @var $phpbb_content_visibility \phpbb\content_visibility */
        $phpbb_content_visibility = $phpbb_container->get('content.visibility');
        $phpbb_content_visibility->set_post_visibility($post_visibility, $data_ary['post_id'], $data_ary['topic_id'],
            $data_ary['forum_id'], $user->data['user_id'], time(), '', $is_starter, $is_latest);
    } else {
    ...
    if ($user->data['user_id'] == $poster_id) {
        if (!$data_ary['notify_set'] && $data_ary['notify']) {
            ...
            if (($config['email_enable'] || $config['jab_enable']) && $data_ary['notify_set'] && !$data_ary['notify']) {
    ...
    if ($mode == 'post' || $mode == 'reply' || $mode == 'quote' || $mode == 'at') {
    ...
    // Send Notifications
    $notification_data = array_merge($data_ary, [
     ...
     * @since 3.2.4-RC1
     */
    $vars = ['notification_data', 'data_ary', 'mode', 'poster_id'];
...
//        var_dump($mode);die; // quote // string(2) "at"
        ...
        switch ($mode) {
            ...
            case 'edit_last_post':
                if ($user->data['user_id'] == $poster_id) {
                    $phpbb_notifications->update_notifications([
                        'notification.type.quote',
                    ], $notification_data);
                }
            ...
            if ($post_visibility == ITEM_REAPPROVE) {
                ...
                switch ($mode) {
                    ...
                    case 'edit_first_post':
                        ...
                        $phpbb_notifications->delete_notifications('notification.type.approve_post',
    ...
    if ($post_visibility == ITEM_APPROVED ||
        ...
        ($auth->acl_get('m_approve', $data_ary['forum_id']) && in_array($post_visibility,
                [ITEM_UNAPPROVED, ITEM_REAPPROVE]))) {
...
function phpbb_bump_topic($forum_id, $topic_id, $post_data, $bump_time = false)
{
    ...
    $phpbb_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_BUMP_TOPIC', false, [
        'forum_id' => $forum_id,
        'topic_id' => $topic_id,
        $post_data['topic_title']
    ]);

    $url = append_sid("{$phpbb_root_path}viewtopic.$phpEx",
            "f=$forum_id&amp;t=$topic_id&amp;p={$post_data['topic_last_post_id']}")."#p{$post_data['topic_last_post_id']}";
...
function phpbb_handle_post_delete($forum_id, $topic_id, $post_id, &$post_data, $is_soft = false, $delete_reason = '')
{
    ...
    if ($force_delete_allowed || ($is_soft && $force_softdelete_allowed) || $auth->acl_get("m_$perm_check",
            $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get("f_$perm_check",
                $forum_id) && $post_id == $post_data['topic_last_post_id'] && !$post_data['post_edit_locked'] && ($post_data['post_time'] > time() - ($config['delete_time'] * 60) || !$config['delete_time']))) {
        $s_hidden_fields = [
            'p' => $post_id,
            'f' => $forum_id,
        ...
        if (confirm_box(true)) {
            ...
            if ($next_post_id === false) {
                $phpbb_log->add('mod', $user->data['user_id'], $user->ip,
                    (($is_soft) ? 'LOG_SOFTDELETE_TOPIC' : 'LOG_DELETE_TOPIC'), false, [
                        'forum_id' => $forum_id,
                        'topic_id' => $topic_id,
                        $post_data['topic_title'],
                        $post_username,
                ...
                $phpbb_log->add('mod', $user->data['user_id'], $user->ip,
                    (($is_soft) ? 'LOG_SOFTDELETE_POST' : 'LOG_DELETE_POST'), false, [
                        'forum_id' => $forum_id,
                        'topic_id' => $topic_id,
                        'post_id' => $post_id,
                ...
                $meta_info = append_sid("{$phpbb_root_path}viewtopic.$phpEx",
                        "f=$forum_id&amp;t=$topic_id&amp;p=$next_post_id")."#p$next_post_id";
                $message = $user->lang['POST_DELETED'];
            ...
            $can_delete = $force_delete_allowed || ($auth->acl_get('m_delete',
                        $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get('f_delete',
                            $forum_id)));
            $can_softdelete = $force_softdelete_allowed || ($auth->acl_get('m_softdelete',
                        $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get('f_softdelete',
                            $forum_id)));
    ...
    if ($post_data['poster_id'] == $user->data['user_id'] && $auth->acl_get('f_delete',
            $forum_id) && $post_id != $post_data['topic_last_post_id']) {
