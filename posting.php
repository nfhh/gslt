...
if ($mode == 'delete' && (($confirm && !$request->is_set_post('delete_permanent')) || !$auth->acl_gets('f_delete',
            'm_delete', $forum_id))) {
    $mode = 'soft_delete';
}
...
if ($cancel) {
    ...
    $redirect = ($post_id) ? append_sid("{$phpbb_root_path}viewtopic.$phpEx",
            $f.'p='.$post_id).'#p'.$post_id : (($topic_id) ? append_sid("{$phpbb_root_path}viewtopic.$phpEx",
        $f.'t='.$topic_id) : (($forum_id) ? append_sid("{$phpbb_root_path}viewforum.$phpEx",
...
if ($auth->acl_get('m_approve',
        $forum_id) && ((($mode == 'reply' || $mode == 'bump') && $post_data['topic_visibility'] != ITEM_APPROVED) || ($mode == 'quote' && $post_data['post_visibility'] != ITEM_APPROVED))) {
...
$user->setup(['posting', 'mcp', 'viewtopic'], $post_data['forum_style']);
...
switch ($mode) {
    ...
    case 'delete':
        if ($user->data['is_registered'] && ($auth->acl_get('m_delete',
                    $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $auth->acl_get('f_delete',
                        $forum_id)))) {
            $is_authed = true;
        }
    ...
    case 'soft_delete':
        if (!$is_authed && $user->data['is_registered'] && $phpbb_content_visibility->can_soft_delete($forum_id,
                $post_data['poster_id'], $post_data['post_edit_locked'])) {
...
if ($post_data['forum_type'] != FORUM_POST && in_array($mode, ['post', 'bump', 'quote', 'reply'])) {
    trigger_error('USER_CANNOT_FORUM_POST');
}

// Forum/Topic locked?
if (($post_data['forum_status'] == ITEM_LOCKED || (isset($post_data['topic_status']) && $post_data['topic_status'] == ITEM_LOCKED)) && !$auth->acl_get('m_edit',
        $forum_id)) {
...
if ($mode == 'delete' || $mode == 'soft_delete') {
    ...
    phpbb_handle_post_delete($forum_id, $topic_id, $post_id, $post_data,
        ($mode == 'soft_delete' && !$request->is_set_post('delete_permanent')), $delete_reason);
    return;
}
...
if ($mode == 'bump') {
    if ($bump_time = bump_topic_allowed($forum_id, $post_data['topic_bumped'], $post_data['topic_last_post_time'],
            $post_data['topic_poster'], $post_data['topic_last_poster_id'])
        ...
        if (!$request->is_ajax()) {
            ...
            $message .= '<br /><br />'.$user->lang('RETURN_FORUM',
                    '<a href="'.append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f='.$forum_id).'">', '</a>');
        }
...
$post_data['post_subject'] = (in_array($mode, [
    'quote', 'edit'
...
$post_data['icon_id'] = (!isset($post_data['icon_id']) || in_array($mode,
        ['quote', 'reply'])) ? 0 : (int) $post_data['icon_id'];
...
$uninit = [
    'post_attachment' => 0, 'poster_id' => $user->data['user_id'], 'enable_magic_url' => 0, 'topic_status' => 0,
    'topic_type' => POST_NORMAL, 'post_subject' => '', 'topic_title' => '', 'post_time' => 0, 'post_edit_reason' => '',
    'notify_set' => 0
];
...
if ($post_data['poster_id'] == ANONYMOUS) {
    ...
    $post_data['username'] = ($mode == 'quote' || $mode == 'at' || $mode == 'edit') ? trim($post_data['username']) : '';
}
...
if ($mode == 'post') {
    $post_data['topic_status'] = ($request->is_set_post('lock_topic') && $auth->acl_gets('m_lock', 'f_user_lock',
            $forum_id)) ? ITEM_LOCKED : ITEM_UNLOCKED;
}

$post_data['enable_magic_url'] = $post_data['drafts'] = false;

// User own some drafts?
if ($user->data['is_registered'] && $auth->acl_get('u_savedrafts') && ($mode == 'reply' || $mode == 'post' || $mode == 'quote' || $mode == 'at')) {
    $sql = 'SELECT draft_id
		FROM '.DRAFTS_TABLE.'
...
if ($save && $user->data['is_registered'] && $auth->acl_get('u_savedrafts') && ($mode == 'reply' || $mode == 'post' || $mode == 'quote')) {
    ...
    if ($subject && $message) {
        if (confirm_box(true)) {
            ...
            $meta_info = ($mode == 'post') ? append_sid("{$phpbb_root_path}viewforum.$phpEx",
                'f='.$forum_id) : append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id");

            meta_refresh(3, $meta_info);

            $message = $user->lang['DRAFT_SAVED'].'<br /><br />';
            $message .= ($mode != 'post') ? sprintf($user->lang['RETURN_TOPIC'], '<a href="'.$meta_info.'">',
                    '</a>').'<br /><br />' : '';
...
if ($submit || $preview || $refresh) {
    ...
    $post_data['post_edit_reason'] = ($request->variable('edit_reason', false, false,
            \phpbb\request\request_interface::POST) && $mode == 'edit' && $auth->acl_get('m_edit',
            $forum_id)) ? $request->variable('edit_reason', '', true) : '';

    $post_data['orig_topic_type'] = $post_data['topic_type'];
    $post_data['topic_type'] = $request->variable('topic_type',
        (($mode != 'post') ? (int) $post_data['topic_type'] : POST_NORMAL));
    $post_data['topic_time_limit'] = $request->variable('topic_time_limit',
        (($mode != 'post') ? (int) $post_data['topic_time_limit'] : 0));
    ...
    $post_data['enable_sig'] = (!$config['allow_sig'] || !$auth->acl_get('f_sigs',
            $forum_id) || !$auth->acl_get('u_sig')) ? false : ((isset($_POST['attach_sig']) && $user->data['is_registered']) ? true : false);
    ...
    if ($poll_delete && $mode == 'edit' && count($post_data['poll_options']) &&
        ((!$post_data['poll_last_vote'] && $post_data['poster_id'] == $user->data['user_id'] && $auth->acl_get('f_delete',
                    $forum_id)) || $auth->acl_get('m_delete', $forum_id))) {
        ...
        $post_data['poll_vote_change'] = ($auth->acl_get('f_votechg', $forum_id) && $auth->acl_get('f_vote',
                $forum_id) && isset($_POST['poll_vote_change'])) ? 1 : 0;
    }
    ...
    if ($mode != 'edit' && !$preview && !$refresh && $config['flood_interval'] && !$ignore_flood) {
        ...
        if ($user->data['username'] !== 'TerraSupport' && $last_post_time && ($current_time - $last_post_time) < intval($config['flood_interval'])) {
    ...
    if (($post_data['username'] && !$user->data['is_registered']) || ($mode == 'edit' && $post_data['poster_id'] == ANONYMOUS && $post_data['username'] && $post_data['post_username'] && $post_data['post_username'] != $post_data['username'])) {
        ...
        if (($result = validate_username($post_data['username'],
                (!empty($post_data['post_username'])) ? $post_data['post_username'] : '')) !== false) {
            $error[] = $user->lang[$result.'_USERNAME'];
        }

        if (($result = validate_string($post_data['username'], false, $config['min_name_chars'],
                $config['max_name_chars'])) !== false) {
    ...
    if ($config['enable_post_confirm'] && !$user->data['is_registered'] && in_array($mode,
            ['quote', 'post', 'reply'])) {
        $captcha_data = [
            'message' => $request->variable('message', '', true),
            'subject' => $request->variable('subject', '', true),
    ...
    if ($submit && $mode == 'edit' && $post_data['post_visibility'] == ITEM_DELETED && !isset($_POST['soft_delete']) && $auth->acl_get('m_approve',
            $forum_id)) {
        ...
        $updated_post_data = $phpbb_content_visibility->set_post_visibility(ITEM_APPROVED, $post_id,
            $post_data['topic_id'], $post_data['forum_id'], $user->data['user_id'], time(), '', $is_first_post,
            $is_last_post);
    ...
    if ($post_data['poll_option_text'] &&
        ...
        if ($mode == 'edit' && $post_id == $post_data['topic_first_post_id'] && $auth->acl_get('f_poll', $forum_id)) {
            ...
            if (!$auth->acl_get('f_poll',
                    $forum_id) && ($mode == 'edit') && ($post_id == $post_data['topic_first_post_id']) && !$bbcode_utils->is_empty($original_poll_data['poll_title'])) {
                ...
                $poll = array_merge($original_poll_data, [
                    'enable_bbcode' => $post_data['enable_bbcode'],
                    'enable_urls' => $post_data['enable_urls'],
                    'enable_smilies' => $post_data['enable_smilies'],
                    'img_status' => $img_status,
    ...
    if (!count($error) && $submit) {
        if ($submit) {
            ...
            $perm_lock_unlock = ($auth->acl_get('m_lock', $forum_id) || ($auth->acl_get('f_user_lock',
                        $forum_id) && $user->data['is_registered'] && !empty($post_data['topic_poster']) && $user->data['user_id'] == $post_data['topic_poster'] && $post_data['topic_status'] == ITEM_UNLOCKED)) ? true : false;
            ...
            if ($change_topic_status != $post_data['topic_status']) {
                ...
                $user_lock = ($auth->acl_get('f_user_lock',
                        $forum_id) && $user->data['is_registered'] && $user->data['user_id'] == $post_data['topic_poster']) ? 'USER_' : '';

                $phpbb_log->add('mod', $user->data['user_id'], $user->ip,
                    'LOG_'.$user_lock.(($change_topic_status == ITEM_LOCKED) ? 'LOCK' : 'UNLOCK'), false, [
                        'forum_id' => $forum_id,
                        'topic_id' => $topic_id,
            ...
            if ($mode == 'edit' && $post_data['post_edit_locked'] == ITEM_LOCKED && !$post_lock && $auth->acl_get('m_edit',
                    $forum_id)) {
                $post_data['post_edit_locked'] = ITEM_UNLOCKED;
            } else {
                if ($mode == 'edit' && $post_data['post_edit_locked'] == ITEM_UNLOCKED && $post_lock && $auth->acl_get('m_edit',
                        $forum_id)) {
            ...
            // 添加和编辑@ 都会走这里接到表单 只是添加会rebuild 编辑没有
            $data = [
            ...
            $redirect_url = submit_post($mode, $post_data['post_subject'], $post_author_name, $post_data['topic_type'],
                $poll, $data, $update_message, ($update_message || $update_subject) ? true : false); // 这里保存的
            ...
            if ($request->is_set_post('delete') || $request->is_set_post('delete_permanent')) {
                $delete_reason = $request->variable('delete_reason', '', true);
                phpbb_handle_post_delete($forum_id, $topic_id, $post_id, $post_data,
                    !$request->is_set_post('delete_permanent'), $delete_reason);
                return;
            }

            // Check the permissions for post approval.
            // Moderators must go through post approval like ordinary users.
            if ((!$auth->acl_get('f_noapprove',
                        $data['forum_id']) && empty($data['force_approved_state'])) || (isset($data['force_approved_state']) && !$data['force_approved_state'])) {
                meta_refresh(10, $redirect_url);
                $message = ($mode == 'edit') ? $user->lang['POST_EDITED_MOD'] : $user->lang['POST_STORED_MOD'];
                $message .= (($user->data['user_id'] == ANONYMOUS) ? '' : ' '.$user->lang['POST_APPROVAL_NOTIFY']);
                $message .= '<br /><br />'.sprintf($user->lang['RETURN_FORUM'],
                        '<a href="'.append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f='.$data['forum_id']).'">',
                        '</a>');
                trigger_error($message);
            }
...
if (!count($error) && $preview) {
    ...
    if (!$poll_delete && ($mode == 'post' || ($mode == 'edit' && $post_id == $post_data['topic_first_post_id']/* && (!$post_data['poll_last_vote'] || $auth->acl_get('m_edit', $forum_id))*/))
        ...
        $parse_poll->format_display($post_data['enable_bbcode'], $post_data['enable_urls'],
        ...
        $template->assign_vars([
            ...
            'L_POLL_LENGTH' => ($post_data['poll_length']) ? sprintf($user->lang['POLL_RUN_TILL'],
                $user->format_date($poll_end)) : '',
...
$generate_quote = (($mode == 'quote' || $mode == 'at') && !$submit && !$preview && !$refresh);
...
$post_data['bbcode_uid'] = (($mode == 'quote' || $mode == 'at') && !$preview && !$refresh && !count($error)) ? $post_data['bbcode_uid'] : $message_parser->bbcode_uid;
$message_parser->decode_message($post_data['bbcode_uid']);

if ($generate_quote) {
    // Remove attachment bbcode tags from the quoted message to avoid mixing with the new post attachments if any
    $message_parser->message = preg_replace('#\[attachment=([0-9]+)\](.*?)\[\/attachment\]#uis', '\\2',
    ...
    phpbb_format_quote($language, $message_parser, $bbcode_utils, $bbcode_status, $quote_attributes); // 这里开始有了

}

if (($mode == 'reply' || $mode == 'quote' || $mode == 'at') && !$submit && !$preview && !$refresh) {
    $post_data['post_subject'] = ((strpos($post_data['post_subject'],
                'Re: ') !== 0) ? 'Re: ' : '').censor_text($post_data['post_subject']);
...
$s_hidden_fields .= ($draft_id || isset($_REQUEST['draft_loaded'])) ? '<input type="hidden" name="draft_loaded" value="'.$request->variable('draft_loaded',
        $draft_id).'" />' : '';
...
$form_enctype = (@ini_get('file_uploads') == '0' || strtolower(@ini_get('file_uploads')) == 'off' || !$config['allow_attachments'] || !$auth->acl_get('u_attach') || !$auth->acl_get('f_attach',
        $forum_id)) ? '' : ' enctype="multipart/form-data"';
...
$page_data = [
    ...
    'FORUM_NAME' => $post_data['forum_name'],
    'FORUM_DESC' => ($post_data['forum_desc']) ? generate_text_for_display($post_data['forum_desc'],
    ...
    'BBCODE_STATUS' => $user->lang(($bbcode_status ? 'BBCODE_IS_ON' : 'BBCODE_IS_OFF'),
        '<a href="'.$controller_helper->route('phpbb_help_bbcode_controller').'">', '</a>'),
    ...
    'U_VIEW_TOPIC' => ($mode != 'post') ? append_sid("{$phpbb_root_path}viewtopic.$phpEx",
    ...
    'S_DELETE_ALLOWED' => ($mode == 'edit' && (($post_id == $post_data['topic_last_post_id'] && $post_data['poster_id'] == $user->data['user_id'] && $auth->acl_get('f_delete',
                    $forum_id) && !$post_data['post_edit_locked'] && ($post_data['post_time'] > time() - ($config['delete_time'] * 60) || !$config['delete_time'])) || $auth->acl_get('m_delete',
                $forum_id))) ? true : false,
    'S_BBCODE_ALLOWED' => ($bbcode_status) ? 1 : 0,
    'S_BBCODE_CHECKED' => ($bbcode_checked) ? ' checked="checked"' : '',
    'S_SMILIES_ALLOWED' => $smilies_status,
    'S_SMILIES_CHECKED' => ($smilies_checked) ? ' checked="checked"' : '',
    'S_SIG_ALLOWED' => ($auth->acl_get('f_sigs',
            $forum_id) && $config['allow_sig'] && $user->data['is_registered']) ? true : false,
    ...
    'S_LOCK_TOPIC_ALLOWED' => (($mode == 'edit' || $mode == 'reply' || $mode == 'quote' || $mode == 'post') && ($auth->acl_get('m_lock',
                $forum_id) || ($auth->acl_get('f_user_lock',
                    $forum_id) && $user->data['is_registered'] && !empty($post_data['topic_poster']) && $user->data['user_id'] == $post_data['topic_poster'] && $post_data['topic_status'] == ITEM_UNLOCKED))) ? true : false,
    'S_LOCK_TOPIC_CHECKED' => ($lock_topic_checked) ? ' checked="checked"' : '',
    'S_LOCK_POST_ALLOWED' => ($mode == 'edit' && $auth->acl_get('m_edit', $forum_id)) ? true : false,
    'S_LOCK_POST_CHECKED' => ($lock_post_checked) ? ' checked="checked"' : '',
    'S_SOFTDELETE_CHECKED' => ($mode == 'edit' && $post_data['post_visibility'] == ITEM_DELETED) ? ' checked="checked"' : '',
    'S_SOFTDELETE_ALLOWED' => ($mode == 'edit' && $phpbb_content_visibility->can_soft_delete($forum_id,
            $post_data['poster_id'], $lock_post_checked)) ? true : false,
...
if (($mode == 'post' || ($mode == 'edit' && $post_id == $post_data['topic_first_post_id']/* && (!$post_data['poll_last_vote'] || $auth->acl_get('m_edit', $forum_id))*/))
    ...
    $page_data = array_merge($page_data, [
            ...
            'S_POLL_DELETE' => ($mode == 'edit' && count($post_data['poll_options']) && ((!$post_data['poll_last_vote'] && $post_data['poster_id'] == $user->data['user_id'] && $auth->acl_get('f_delete',
                            $forum_id)) || $auth->acl_get('m_delete', $forum_id))),
            'S_POLL_DELETE_CHECKED' => (!empty($poll_delete)) ? true : false,

            'L_POLL_OPTIONS_EXPLAIN' => $user->lang('POLL_OPTIONS_'.(($mode == 'edit') ? 'EDIT_' : '').'EXPLAIN',
                (int) $config['max_poll_options']),
...
$allowed = ($auth->acl_get('f_attach',
        $forum_id) && $auth->acl_get('u_attach') && $config['allow_attachments'] && $form_enctype);
...
// Topic review
if ($mode == 'reply' || $mode == 'quote' || $mode == 'at') {
